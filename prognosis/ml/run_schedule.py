...
def run_schedule(configs_path : str) -> None:
    ...
    print("START")
    while True:
        configs = [f for f in os.listdir(configs_path) if os.path.isfile(os.path.join(configs_path, f))]
        configs = sorted(configs, key=lambda x: int(x.split("_")[0]))
